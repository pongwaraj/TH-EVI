...
OUT_PATH = Path(r"D:\Work\TH-EVI\artifacts\Wasabi_Park_Area_Analysis_Proposal.docx")
...
def add_title_block(doc: Document) -> None:
    ...
    run = p.add_run("Wasabi Park | เจ้าของโครงการลงทุนเอง")
    run.font.name = "Calibri"
    run.font.size = Pt(13)
    run.font.color.rgb = MUTED
    ...
    rows = [
        ...
        ("ช่วงพยากรณ์", f"{START_YEAR}-{END_YEAR} รวม 9 ปี"),
    ]
...
def add_executive_summary(doc: Document, current_row: dict, payback: tuple[int, float] | None) -> None:
    ...
    text = (
        ...
        f"และการใช้งานจริงมักถูกจำกัดด้วยการหมุนเวียนรถ ช่วงพีค และพื้นที่จอด มากกว่าดีมานด์เชิงทฤษฎีของโมเดล อีกทั้งพื้นที่นี้มีคู่แข่ง EV อยู่ใกล้มาก จึงควรใช้สมมติฐานแบบ conservative แม้พื้นที่โดยรอบยังมีดีมานด์รวมในระดับดี "
...
def add_key_assumptions(doc: Document) -> None:
    ...
    rows = [
        ...
        ("กรอบเวลา", "9 ปี", "ใช้เป็นกรอบ forecast ระยะกลางเพื่อดู cashflow และ payback"),
        ("ดีมานด์ต่อคัน", "35 kWh/คัน", "ตีความจากโจทย์เป็นพลังงานเฉลี่ยต่อคัน"),
        ("ราคาขาย", "10.0 บาท/kWh", "สมมติฐาน Base Case เพื่อคำนวณรายได้"),
        ("ต้นทุนค่าไฟ", "5.0 บาท/kWh", "ใช้เป็นสมมติฐาน Base Case สำหรับประเมินต้นทุนพลังงาน"),
...
def add_area_analysis(doc: Document, base_result: dict) -> None:
    ...
    doc.add_paragraph(
        ...
        "อย่างไรก็ดี จุดนี้มีคู่แข่ง EV อยู่ใกล้มาก จึงควรอ่านผลแบบระมัดระวังและให้ความสำคัญกับการจับลูกค้าหน้างานจริงของ community mall"
    )
...
def add_financial_forecast(doc: Document, rows: list[dict], payback: tuple[int, float] | None) -> None:
    ...
    summary_rows = [
        f"ปีแรก ({START_YEAR}) โมเดลมอง demand ไว้ที่ {fmt_num(rows[0]['modeled_cars_per_day'])} คัน/วัน และ Base Case ใช้ {fmt_num(rows[0]['cars_per_day'])} คัน/วัน "
        f"โดยจุดนี้ไม่ได้ถูก cap ด้วยจำนวนช่องจอดในปีแรก แต่ยังอ่านแบบระมัดระวังเพราะมีคู่แข่งใกล้มาก และให้กระแสเงินสดจากการดำเนินงานหลังหักค่าไฟและ GP ประมาณ {fmt_int(rows[0]['annual_contribution'])} บาท/ปี",
...
def add_caveats(doc: Document, base_result: dict) -> None:
    ...
    bullets = [
        ...
        "ข้อมูลประกอบที่มีอยู่ชี้ว่าไซต์มีฐาน EV เดิมบางส่วนจาก AC charger ภายในโครงการ ซึ่งเป็นสัญญาณเชิงบวกต่อการเริ่มต้น แต่ไม่ได้ถูกแปลงเป็นรายได้เพิ่มอัตโนมัติใน Base Case นี้",
